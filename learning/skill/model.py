...
class SkillModel(StyleModel):
    ...
    class NetworkWrapper(StyleModel.NetworkWrapper):
        ...
        def forward(self, input_dict):
            ...
            if input_dict['is_train']:
                ...
                result.update({
                    ...
                    'entropy': distr.entropy().sum(dim=-1),
                    'values': normalized_value,
                    'rollout_disc_logit': self.a2c_network.eval_disc(input_dict['normalized_rollout_disc_obs']),
                    'replay_disc_logit': self.a2c_network.eval_disc(input_dict['normalized_replay_disc_obs']),
                    'demo_disc_logit': self.a2c_network.eval_disc(input_dict['normalized_demo_disc_obs']),
                    'enc': self.a2c_network.eval_enc(input_dict['normalized_rollout_disc_obs']),
                })
            else:
                selected_action = distr.sample()
                neglogp = self.neglogp(selected_action, mu, sigma, logstd)
        ...
        def enc(self, normalized_obs):
            return self.a2c_network.eval_enc(normalized_obs)
